...
describe("lexer", function()
   ...
   it("parses strings correctly", function()
      ...
      assert.same({ token = "string", token_value = "\\u0000" }, get_token("\"\\u0000\")"))
      assert.same({ token = "string", token_value = "\\ufA16" }, get_token("\"\\ufA16\")"))
      assert.same({ token = "string", token_value = "\\U00000000" }, get_token("\"\\U00000000\")"))
      assert.same({ token = "string", token_value = "\\U0000ffAB" }, get_token("\"\\U0000ffAB\")"))
   ...
   it("parses integers correctly", function()
      ...
      assert.same({ token = "number", token_value = "-0X42" }, get_token("-0X42"))
      assert.same({ token = "number", token_value = "-0X123456789abcDEF" }, get_token("-0X123456789abcDEF"))
      assert.same({ token = "number", token_value = "-0X" }, get_token("-0X"))
   end)
...
EOF
   ...
   it("provides errors on incorrect heredoc syntax", function()
         assert.same({token = "eof",     token_value = nil}, get_token("<<EOF"))
   end)
end)
